...
//moved these from the autonomous case because now I want to conditionally use both autonomous and "driven"
...
int32_t dt = 0L;      // uS elapsed since last eye event
...
/*remove this whole section since I'm not going to report anything back to python, which will be using the serial port
...
*/
  ...
  if (useFaceData) {
    ...
    dt = t - eyeMoveStartTime;              // uS elapsed since last eye event
...
void loop() {
  ...
  //Get serial data and store the x and y position of a face
  while (Serial.available() > 0) {
   
    // look for the next valid integer in the incoming serial stream:
    xFacePos = Serial.parseInt();
    // do it again:
    yFacePos = Serial.parseInt();

    // look for the newline. That's the end of the sentence:
    if (Serial.read() == '\n') {
      useFaceData = true;
      previousMillis = millis(); //reset the 'last good' time
      break;
    }
  }
  ...
  if (currentMillis - previousMillis > idleTime){
    //would like to turn the screens black at least when no data has been received for a long time
    eye[0].display->fillScreen(0x0000);
    eye[1].display->fillScreen(0x0000);
    while(!Serial.available()) {
      delay(100);
    }
  }
  else if (currentMillis - previousMillis > staleTime) {
